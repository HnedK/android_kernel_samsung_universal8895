...
void displayport_get_voltage_and_pre_emphasis_max_reach(u8 *drive_current,
                                                        u8 *pre_emphasis,
                                                        u8 *max_reach_value) {
  int i;
...
Voltage_Swing_Retry:
  ...
  displayport_reg_set_voltage_and_pre_emphasis((u8 *)drive_current,
                                               (u8 *)pre_emphasis);
  displayport_get_voltage_and_pre_emphasis_max_reach(
      (u8 *)drive_current, (u8 *)pre_emphasis, (u8 *)max_reach_value);
  ...
  if (lane_cnt == 0x04) {
    ...
    displayport_reg_dpcd_write(DPCD_ADD_TRANING_PATTERN_SET, 1, val);
    displayport_err(
        "Full Link Training Fail : Link Rate %02x, lane Count %02x -",
        link_rate, lane_cnt);
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
  ...
  if (need_retraining) {
    ...
    displayport_info(
        "check voltage swing(%d): val(%d,%d) %d/%d, %d/%d, %d/%d, %d/%d\n",
        training_retry_no, val[0], val[1], drive_current[0],
        voltage_swing_lane[0], drive_current[1], voltage_swing_lane[1],
        drive_current[2], voltage_swing_lane[2], drive_current[3],
        voltage_swing_lane[3]);
  ...
  if (lane_cnt == 0x04) {
    if (((lane_channel_eq_done & 0x0F) == 0x0F) &&
      ...
      displayport_info("LANE_SET [%d] : %02x %02x %02x %02x\n",
                       eq_training_retry_no, eq_val[0], eq_val[1], eq_val[2],
                       eq_val[3]);
#ifdef CONFIG_SEC_DISPLAYPORT_BIGDATA
...
static int
displayport_read_branch_revision(struct displayport_device *displayport) {
  int ret = 0;
  u8 val[4] = {
      0,
  };
...
static int displayport_link_status_read(void) {
  u8 val[DPCP_LINK_SINK_STATUS_FIELD_LENGTH] = {
      0,
  };
  int count = 10;
  ...
  displayport_reg_dpcd_read_burst(DPCD_ADD_DEVICE_SERVICE_IRQ_VECTOR,
                                  DPCP_LINK_SINK_STATUS_FIELD_LENGTH - 1,
                                  &val[1]);
...
static void displayport_set_switch_state(struct displayport_device *displayport,
                                         int state) {
  if (state) {
...
void displayport_hpd_changed(int state) {
  ...
  if (state) {
    ...
    timeout = wait_event_interruptible_timeout(
        displayport->dp_wait, (displayport->state == DISPLAYPORT_STATE_ON),
    ...
    timeout = wait_event_interruptible_timeout(
        displayport->dp_wait, (displayport->state == DISPLAYPORT_STATE_INIT),
        msecs_to_jiffies(1000));
    if (!timeout)
...
static int displayport_Automated_Test_Request(void) {
  ...
  if ((data & TEST_LINK_TRAINING) == TEST_LINK_TRAINING) {
    ...
    displayport->dyn_range =
        (val[0] & TEST_DYNAMIC_RANGE) ? CEA_RANGE : VESA_RANGE;
...
static void displayport_hpd_irq_work(struct work_struct *work) {
  ...
  if (displayport->hdcp_ver == HDCP_VERSION_2_2) {
    int ret = displayport_reg_dpcd_read_burst(
        DPCD_ADD_SINK_COUNT, DPCP_LINK_SINK_STATUS_FIELD_LENGTH, val);

    displayport_info("HPD IRQ work %02x %02x %02x %02x %02x %02x\n", val[0],
                     val[1], val[2], val[3], val[4], val[5]);
    if (ret < 0 ||
        (val[0] & val[1] & val[2] & val[3] & val[4] & val[5]) == 0xff) {
    ...
    if ((val[1] & CP_IRQ) == CP_IRQ) {
      ...
      displayport_reg_dpcd_write(DPCD_ADD_DEVICE_SERVICE_IRQ_VECTOR, 1,
                                 &cpirq_bit);
  ...
  if (hdcp13_info.auth_state != HDCP13_STATE_AUTH_PROCESS) {
    ...
    if ((val[1] & CP_IRQ) == CP_IRQ &&
        displayport->hdcp_ver == HDCP_VERSION_1_3) {
      u8 cpirq_bit = CP_IRQ;
...
static irqreturn_t displayport_irq_handler(int irq, void *dev_data) {
  ...
  irq_status_reg =
      displayport_reg_get_interrupt_and_clear(Common_Interrupt_Status_2);

  if (irq_status_reg & HDCP_LINK_CHK_FAIL) {
    queue_delayed_work(displayport->dp_wq,
                       &displayport->hdcp13_integrity_check_work, 0);
  ...
  irq_status_reg =
      displayport_reg_get_interrupt_and_clear(Common_Interrupt_Status_4);

  if (irq_status_reg & HPD_CHG)
    displayport_info("HPD_CHG detect\n");

  if (irq_status_reg & HPD_LOST) {
    /*queue_delayed_work(displayport->dp_wq, &displayport->hpd_unplug_work,
     * 0);*/
...
static int
displayport_make_avi_infoframe_data(struct infoframe *avi_infoframe) {
  int i;
...
static int displayport_make_audio_infoframe_data(
  ...
  displayport_info("audio_infoframe: type and ch_cnt %02x, SF and bit size "
                   "%02x, ch_allocation %02x\n",
                   audio_infoframe->data[0], audio_infoframe->data[1],
                   audio_infoframe->data[3]);
...
static int
displayport_make_spd_infoframe_data(struct infoframe *avi_infoframe) {
...
static int displayport_set_audio_infoframe(
    struct displayport_audio_config_data *audio_config_data) {
  struct infoframe audio_infoframe;
...
static int displayport_set_audio_ch_status(
    struct displayport_audio_config_data *audio_config_data) {
...
int displayport_audio_config(
    struct displayport_audio_config_data *audio_config_data) {
  int ret = 0;

  displayport_info(
      "audio en:%d, ch:%d, fs:%d, bit:%d, packed:%d, word_len:%d\n",
  ...
  displayport_reg_set_audio_fifo_function_enable(
      audio_config_data->audio_enable);
...
int displayport_audio_bist_enable(
    struct displayport_audio_config_data audio_config_data) {
  int ret = 0;
...
static int displayport_check_hdcp_version(void) {
...
#endif
  displayport_dpcd_read_for_hdcp22(DPCD_HDCP22_RX_CAPS,
                                   DPCD_HDCP22_RX_CAPS_LENGTH, val);
  ...
  if ((((rx_caps & VERSION) >> DPCD_HDCP_VERSION_BIT_POSITION) ==
       (HDCP_VERSION_2_2)) &&
...
static int displayport_enable(struct displayport_device *displayport) {
  ...
  if (displayport->bist_used)
    displayport_reg_set_bist_video_configuration(g_displayport_videoformat, bpc,
                                                 bist_type, dyn_range);
  else {
    if (displayport->dfp_type != DFP_TYPE_DP)
      bpc = BPC_8;
...
bool displayport_match_timings(const struct v4l2_dv_timings *t1,
  ...
  if (t1->bt.width == t2->bt.width && t1->bt.height == t2->bt.height &&
      ...
      (!t1->bt.interlaced || (t1->bt.il_vfrontporch == t2->bt.il_vfrontporch &&
...
bool displayport_match_video_params(u32 i, u32 j) {
  if (displayport_supported_presets[i].xres ==
          videoformat_parameters[j].active_pixel &&
      displayport_supported_presets[i].yres ==
          videoformat_parameters[j].active_line &&
...
static void displayport_dv_timings_to_str(const struct v4l2_dv_timings *t,
                                          char *str, int size) {
  ...
  fps = (htot * vtot) > 0 ? div_u64((100 * (u64)bt->pixelclock), (htot * vtot))
                          : 0;
...
static int displayport_s_dv_timings(struct v4l2_subdev *sd,
  ...
  struct displayport_device *displayport =
      container_of(sd, struct displayport_device, sd);
...
static int displayport_enum_dv_timings(struct v4l2_subdev *sd,
  ...
  if (displayport->dex_setting) {
    if (displayport_supported_presets[timings->index].dex_support ==
        DEX_NOT_SUPPORT) {
    ...
    if (displayport_supported_presets[timings->index].dex_support >
      ...
      displayport_info(
          "%s not supported, adapter:%d, resolution:%d in dex mode\n",
          displayport_supported_presets[timings->index].name,
          displayport->dex_adapter_type,
  ...
  if (displayport_check_bandwidth(displayport_supported_presets[timings->index]
  ...
  if (reduced_resolution &&
      reduced_resolution < displayport_supported_presets[timings->index]
                               .dv_timings.bt.pixelclock) {
...
static long displayport_ioctl(struct v4l2_subdev *sd, unsigned int cmd,
                              void *arg) {
  struct displayport_device *displayport =
      container_of(sd, struct displayport_device, sd);
  int ret = 0;
...
static int displayport_parse_dt(struct displayport_device *displayport,
                                struct device *dev) {
  ...
  if (of_property_read_string(np, "dp,aux_vdd", &displayport->aux_vdd) < 0) {
...
static int displayport_init_resources(struct displayport_device *displayport,
  ...
  ret = devm_request_irq(displayport->dev, res->start, displayport_irq_handler,
                         0, pdev->name, displayport);
  if (ret) {
...
static int displayport_aux_onoff(struct displayport_device *displayport,
                                 int onoff) {
  int rc = 0;
...
static void
displayport_check_adapter_type(struct displayport_device *displayport) {
...
static int usb_typec_displayport_notification(struct notifier_block *nb,
                                              ...
                                              void *data) {
  struct displayport_device *displayport =
      container_of(nb, struct displayport_device, dp_typec_nb);
  ...
  displayport_dbg(
      ...
      usb_typec_info.id, usb_typec_info.sub1, usb_typec_info.sub2,
      usb_typec_info.sub3);
...
static CLASS_ATTR(bpc, 0664, displayport_test_bpc_show,
                  displayport_test_bpc_store);
...
static ssize_t displayport_test_range_show(struct class *class,
  ...
  return sprintf(buf, "displayport range %s\n",
                 (displayport->dyn_range == VESA_RANGE) ? "VESA_RANGE"
                                                        : "CEA_RANGE");
}
...
static CLASS_ATTR(range, 0664, displayport_test_range_show,
                  displayport_test_range_store);
...
static ssize_t displayport_test_edid_store(struct class *dev,
  ...
  pr_info("displayport preferred_preset = %d %d %d\n",
          videoformat_parameters[i].active_pixel,
          videoformat_parameters[i].active_line, videoformat_parameters[i].fps);

  return size;
}
static CLASS_ATTR(edid, 0664, displayport_test_edid_show,
                  displayport_test_edid_store);
...
static ssize_t displayport_test_bist_show(struct class *class,
  ...
  return snprintf(buf, PAGE_SIZE, "displayport bist used %d type %d\n",
                  displayport->bist_used, displayport->bist_type);
}
...
static CLASS_ATTR(bist, 0664, displayport_test_bist_show,
                  displayport_test_bist_store);
...
static CLASS_ATTR(audio_test, 0664, displayport_audio_test_show,
                  displayport_audio_test_store);
...
static ssize_t displayport_dp_test_show(struct class *class,
  ...
  size +=
      snprintf(buf + size, PAGE_SIZE - size, "2: CTS power management test\n");
  size +=
      snprintf(buf + size, PAGE_SIZE - size, "3: set lane count, link rate\n");
...
static CLASS_ATTR(dp_test, 0664, displayport_dp_test_show,
                  displayport_dp_test_store);
...
static ssize_t
displayport_reduced_resolution_store(struct class *dev,
                                     struct class_attribute *attr,
                                     const char *buf, size_t size) {
  int val[4] = {
      0,
...
static ssize_t displayport_dex_store(struct class *dev,
  ...
  displayport_info("dex state:%d, setting:%d, run:%d, hpd:%d\n",
                   displayport->dex_state, displayport->dex_setting, dex_run,
                   displayport->hpd_current_state);
  ...
  if (!displayport->notifier_registered) {
    ...
    manager_notifier_register(&displayport->dp_typec_nb,
                              usb_typec_displayport_notification,
  ...
  if (displayport->dex_setting) {
    ...
    if (displayport_supported_presets[g_displayport_videoformat].dex_support !=
            DEX_NOT_SUPPORT &&
        displayport_supported_presets[g_displayport_videoformat].dex_support <=
            displayport->dex_adapter_type) {
...
static ssize_t displayport_aux_sw_sel_store(struct class *dev,
  ...
  if ((aux_sw_sel == 0 || aux_sw_sel == 1) &&
      (aux_sw_oe == 0 || aux_sw_oe == 1)) {
    if (gpio_is_valid(displayport->gpio_sw_sel))
...
static CLASS_ATTR(log_level, 0664, displayport_log_level_show,
                  displayport_log_level_store);
...
static ssize_t displayport_dpcd_write(unsigned int offset, void *buffer,
                                      size_t size) {
  int ret;
...
static ssize_t displayport_dpcd_read(unsigned int offset, void *buffer,
                                     size_t size) {
  int ret;
...
static int displayport_probe(struct platform_device *pdev) {
  ...
  init_dp_logger();
  displayport =
      devm_kzalloc(dev, sizeof(struct displayport_device), GFP_KERNEL);
  if (!displayport) {
  ...
  INIT_DELAYED_WORK(&displayport->hdcp13_integrity_check_work,
                    displayport_hdcp13_integrity_check_work);
  ...
  secdp_aux_dev_init(displayport_i2c_write, displayport_i2c_read,
                     displayport_dpcd_write, displayport_dpcd_read,
                     displayport_get_hpd_state);
